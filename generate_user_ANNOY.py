...
parser = argparse.ArgumentParser(description="Generate the sbatch files for the user-based experiments with ANNOY.")
...
for k in neighbors:
    for t in n_trees:
        for s in search_k:
            with open(f"config_run_experiments_yelp/user_ANNOY_experiment_yelp_neighbors={k}_n_trees={t}_search_k={s}.yml", "w") as f:
...
python script_run_experiment_annoy_generic.py --type user --neighbors {neighbors} --n_trees {n_trees} --search_k {search_k}
"""
# create the directory for the sbatch files
os.makedirs("sbatch_files_user_ANNOY", exist_ok=True)
...
for k in neighbors:
    for t in n_trees:
        for s in search_k:
            ...
            sbatch_file_path = f"sbatch_files_user_ANNOY/run_annoy_user_n={k}_t={t}_k={s}.sbatch"

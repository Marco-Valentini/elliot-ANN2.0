...
user_80 = np.quantile(user_frequencies['Frequency'], 0.80)
item_80 = np.quantile(item_frequencies['Frequency'], 0.80)

user_bins = [0, user_80, np.inf]
...
# user_frequencies[['UserID, 'group']].to_csv(f'data/{dataset}/user_groups_2.tsv', sep='\t', header=None, index=None)
# item_frequencies[['ItemID, 'group']].to_csv(f'data/{dataset}/item_groups_2.tsv', sep='\t', header=None, index=None)
...
with open(f'data/{dataset}/group2_to_users.json', 'w') as file:
    json.dump(group_to_users, file, indent=4)

with open(f'data/{dataset}/group2_to_items.json', 'w') as file:

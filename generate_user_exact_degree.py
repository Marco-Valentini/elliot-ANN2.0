...
template = """experiment:
  ...
  top_k: 50
  path_output_rec_result: ./results/yelp_user_ed/recs/
  path_output_rec_weight: ./results/yelp_user_ed/weights/
  path_output_rec_performance: ./results/yelp_user_ed/performance/
  path_log_folder: ../log/yelp_user_ed/
  evaluation:
    cutoffs: 20
...
n_hash = [ [2, 3], [4, 5] ]
sampling_strategy = [ "exact_degree" ]
neighbors = [ 50, 100, 250, 500 ]

for n in n_hash:
    for s in sampling_strategy:
        for k in neighbors:
            with open(f"config_run_exact_degree/user_experiment_yelp_n_hash={n}_sampling={s}_neighbors={k}.yml", "w") as f:
...
#SBATCH --output=user_out_ed/log_user_[{n_hash[0]}_{n_hash[1]}]_{sampling_strategy}_{neighbors}.out
#SBATCH --error=user_err_ed/log_user_[{n_hash[0]}_{n_hash[1]}]_{sampling_strategy}_{neighbors}.err
...
python script_run_generic.py --config_path config_run_exact_degree/user_experiment_yelp_n_hash={n_hash}_sampling={sampling_strategy}_neighbors={neighbors}.yml
"""
# create the directory for the sbatch files
os.makedirs("sbatch_files_user_ed", exist_ok=True)

...
template = """experiment:
  ...
  top_k: 50
  path_output_rec_result: ./results/yelp_item_ed/recs/
  path_output_rec_weight: ./results/yelp_item_ed/weights/
  path_output_rec_performance: ./results/yelp_item_ed/performance/
  path_log_folder: ../log/yelp_item_ed/
  evaluation:
    cutoffs: 20
...
for n in n_hash:
    for s in sampling_strategy:
        for k in neighbors:
            with open(f"config_run_exact_degree/item_experiment_yelp_n_hash={n}_sampling={s}_neighbors={k}.yml",
                      "w") as f:
...
#SBATCH --output=item_out_ed/log_item_[{n_hash[0]}_{n_hash[1]}]_{sampling_strategy}_{neighbors}.out
#SBATCH --error=item_err_ed/log_item_[{n_hash[0]}_{n_hash[1]}]_{sampling_strategy}_{neighbors}.err
...
os.makedirs("sbatch_files_item_ed", exist_ok=True)
...
for n in n_hash:
    for s in sampling_strategy:
        for k in neighbors:
            ...
            sbatch_file_path = f"sbatch_files_item_ed/run_item_[{n[0]}_{n[1]}]_{s}_{k}.sbatch"
            i += 1

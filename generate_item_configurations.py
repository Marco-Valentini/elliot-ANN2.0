...
for n in n_hash:
    for s in sampling_strategy:
        for k in neighbors:
            with open(f"config_run_experiments_yelp/item_experiment_yelp_n_hash={n}_sampling={s}_neighbors={k}.yml", "w") as f:
...
#SBATCH --output=item_out/log_item_[{n_hash[0]}_{n_hash[1]}]_{sampling_strategy}_{neighbors}.out
#SBATCH --error=item_err/log_item_[{n_hash[0]}_{n_hash[1]}]_{sampling_strategy}_{neighbors}.err
...
os.makedirs("sbatch_files_item", exist_ok=True)

...
from elliot.recommender.ann.user_ann_lsh.user_ann_lsh_similarity import ANNLSHSimilarity
from elliot.recommender.base_recommender_model import BaseRecommenderModel
from elliot.recommender.base_recommender_model import init_charger


class UserANNLSH(RecMixin, BaseRecommenderModel):
    r"""
    ANN recommendations: user-to-user collaborative filtering, with equal approximation for all the users in the neighborhood
    ...
    def __init__(self, data, config, params, *args, **kwargs):
        ...
        self._model = ANNLSHSimilarity(data=self._data, num_neighbors=self._num_neighbors, similarity=self._similarity,
                                    implicit=self._implicit, validate=self._validate, n_hash=self._n_hash, n_tables=self._n_tables,
    ...
    @property
    def name(self):
        return f"UserANNLSH_{self.get_params_shortcut()}"

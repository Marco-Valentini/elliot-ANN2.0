  ...
  models:
    ItemANNOY:
      meta:
        ...
        hyper_opt_alg: grid
      neighbors: {neighbors}
      similarity: angular # hamming distance is the most similar to the jaccard one
      n_trees: {n_trees}
      search_k: {search_k}  
"""
...
n_trees = [1,3,5]
search_k = [-1,5,10]
combinations_to_be_skipped = [(500, 3, -1), (500, 5, 5), (250, 5, -1), (50, 3, 5)]

for k in neighbors:
    for t in n_trees:
        for s in search_k:
            # skip the configurations for which we have already the results
            if (k, t, s) in combinations_to_be_skipped:
                continue
            else:
                with open(f"config_run_experiments_yelp/remaining_item_ANNOY_experiment_yelp_neighbors={k}_n_trees={t}_search_k={s}.yml", "w") as f:
...
#SBATCH --output=item_remaining_annoy_out/log_annoy_item_n={neighbors}_t={n_trees}_k={search_k}.out
#SBATCH --error=item_remaining_annoy_err/log_annoy_item_n={neighbors}_t={n_trees}_k={search_k}.err
...
python script_run_experiment_annoy_generic.py --type remaining_item --neighbors {neighbors} --n_trees {n_trees} --search_k {search_k}
"""

# create the directory for the sbatch files
os.makedirs("remaining_sbatch_files_item_ANNOY", exist_ok=True)
    ...
    for t in n_trees:
        for s in search_k:
            ...
            if (k, t, s) in combinations_to_be_skipped:
                ...
                sbatch_file_path = f"remaining_sbatch_files_item_ANNOY/run_annoy_item_n={k}_t={t}_k={s}.sbatch"
